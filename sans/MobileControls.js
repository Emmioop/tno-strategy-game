...
function update_area() {
    ...
    if (!sc) {
        stage_ready = false;
        if (!game_area || game_area.scale <= 0) {
            game_area = { x: 0, y: 0, w: WORLD_W, h: WORLD_H, scale: 1 };
        }
        ensure_canvas_alive();
        return true;
    }
...
function init() {
    ...
    function tryInitKeyboard() {
        ...
        initAttempts++;
        if (vm && vm.runtime) {
            system_ready = true;  // VM就绪，系统初始化完成
            // 键盘实例的创建不再被 stage_ready 限制，但 game_loop 会自行判断是否执行
            ...
            system_ready = true;  // 即使VM未找到也允许操作，避免完全卡死
            if (ui_state !== 1) {
                ui_state = 2;
