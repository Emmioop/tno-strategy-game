...
function obj_mobilekey_Step_0(self) {
    ...
    for(let i=0;i<4;i++) {
        ...
        // ★ 更新按钮触摸检测坐标（匹配新的阶梯布局）
        else if (gx>=600 && gx<=720 && gy>=260 && gy<=370) { cz=0.5; keyboard_key_press(self.mk_z); }
        else if (gx>=500 && gx<=610 && gy>=320 && gy<=420) { cx=0.5; keyboard_key_press(self.mk_x); }
        else if (gx>=400 && gx<=500 && gy>=370 && gy<=470) { cg=0.5; keyboard_key_press(self.mk_c); }
...
function obj_mobilekey_Draw_75(self) {
    ...
    // Z 按钮 - 青色 (最右上)
    btn._label = 'Z';
    btn(sx(600), sy(260), sx(720), sy(370), gms_col(self.zCol));
    // X 按钮 - 橙色 (中间右上)
    btn._label = 'X';
    btn(sx(500), sy(320), sx(610), sy(420), gms_col(self.xCol));
    // C 按钮 - 绿色 (中间)
    btn._label = 'C';
    btn(sx(400), sy(370), sx(500), sy(470), gms_col(self.cCol));
