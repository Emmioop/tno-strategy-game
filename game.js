...
const Game = {
  ...
  getEventsForTurn() {
    ...
    for (const ev of pool) {
      ...
      if (this.classifyEvent(ev) === 'flavor') flavor.push(ev);
      else core.push(ev);
    }
    ...
    // 4. 核心事件按方向优先级排序: 国内 > 俄罗斯 > 美国 > 日本 > 其他
    const dirOrder = { internal: 0, russia: 1, us: 2, japan: 3, other: 4 };
    core.sort((a, b) =>
      (dirOrder[this.classifyDirection(a)] ?? 9) - (dirOrder[this.classifyDirection(b)] ?? 9)
    );
  ...
  // ===== 事件分类: 风味 vs 核心政治 =====
  classifyEvent(ev) {
    if (!ev) return 'core_internal';
    // 显式标记优先
    if (ev.category === 'flavor') return 'flavor';
    if (ev.category && ev.category.startsWith('core_')) return ev.category;
    const t = ((ev.title || '') + ' ' + (ev.id || '') + ' ' + (ev.body || '')).toLowerCase();
    // 风味: 太空/登月/奥运/体育/电影/音乐/庆典/阅兵/流行病/寒冬/丰收/节日/文化/时尚/科技奇观
    const flavorKw = ['登月','太空','月球','宇航员','航天','卫星','火星','空间站','olympic','奥运','世界杯','电影节','格莱美','奥斯卡','阅兵','庆典','国庆','新年','圣诞','复活节','流行病','流感','疫情','寒冬','酷暑','丰收','饥荒','地震','火山','台风','飓风','时尚','流行歌','摇滚','朋克','电影','电视','广播','文学','小说','绘画','雕塑','建筑展','世博会','万国博览会','展览','颁奖','星光','红毯','票房','唱片','单曲','专辑','音乐会','巡演','博物馆','美术馆','文化遗产','考古','古籍','教堂','大教堂','修道院','朝圣','教皇','教廷','梵蒂冈','节日','国庆日','纪念日','独立日','革命日','加冕','登基','婚礼','葬礼','国葬','周年','百年','世纪','千禧'];
    for (const k of flavorKw) if (t.includes(k)) return 'flavor';
    // 核心方向
    if (/(日本|共荣圈|东京|昭和|裕仁|三头同盟|满洲|朝鲜|台湾|中国|亚洲|太平洋|珍珠港|广岛|长崎|冲绳|japan|co-prosperity)/.test(t)) return 'core_japan';
    if (/(美国|ofn|肯尼迪|约翰逊|尼克松|里根|华盛顿|纽约|洛杉矶|旧金山|阿拉斯加|阿留申|america|united states|us |usa |美国内战|civil rights|民权|马丁路德金|marshall|马歇尔|核谈判|核裁军|nuclear talk|salt |arms control|军控)/.test(t)) return 'core_us';
    if (/(俄罗斯|苏联|西俄|乌克兰|克里米亚|莫斯科|列宁格勒|西伯利亚|鄂木斯克|托木斯克|科米|阿尔汉格尔斯克|阿斯特拉罕|russia|soviet|kiev|ukraine|siberia|november|tabary|塔博|勃列|日丹|托洛|克格勃|kgb|革命阵线|白军|红军|哥萨克)/.test(t)) return 'core_russia';
    // 国内
  ...
  classifyDirection(ev) {
    const cat = this.classifyEvent(ev);
    if (cat === 'flavor') return 'flavor';
    if (cat === 'core_japan') return 'japan';
    if (cat === 'core_us') return 'us';
    if (cat === 'core_russia') return 'russia';
    if (cat === 'core_other') return 'other';
    return 'internal';
  },
  ...
  autoResolveFlavorEvent(ev) {
    // 选第一个可用选项; 若无则跳过
    const choice = (ev.choices && ev.choices.length) ? ev.choices[0] : null;
    if (choice) {
    ...
    this.state.flavorLog.unshift({
      ...
      title: ev.title,
      resolved: choice ? choice.text : '(自动归档)'
    });
    if (this.state.flavorLog.length > 60) this.state.flavorLog.pop();
    // 简短提示
    this.addNews('时代风貌 · ' + (ev.title || ev.id || ''), 'flavor');
    return true;
  },
  ...
  chooseEventOption(ev, choice) {
    ...
    this.state.eventLog.unshift({
      ...
      title: ev.title,
      choice: choice.text,
      category: this.classifyDirection(ev)
    });
